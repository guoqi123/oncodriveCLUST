...
def write_element_results(results, dir, file):
...
def write_cluster_results(results, dir, file, sorter):
    """Save results to the output file
    :param results: dict, dictionary of results, keys are element's symbols
    :param dir: str, output directory
    :param file: str, output file, if elements in elements file
...
def main(input_file,
    ...
    # Write results
    elements_path = write_element_results(results=elements_results, dir=output_directory, file=output_file)
    logger.info('Gene results calculated')
    df = pd.read_csv(elements_path, sep='\t')
    sorted_list_elements = df['SYM'].tolist()
    write_cluster_results(results=clusters_results, dir=output_directory, file=output_file, sorter=sorted_list_elements)
