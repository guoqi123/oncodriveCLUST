...
class Signature:
    ...
    def __init__(self, kmer, genome, start_at_0=False, mutation_type='subs', groupby=False):
        self.kmer = kmer
        self.genome = genome
    ...
    def save(self, directory, prefix):
        ...
        Returns:
            None
        """
        for group, values in self.signatures.items():
            if group == '0':
                output_file = os.path.join(directory, '{}_kmer_{}.pickle'.format(prefix, self.kmer))
            else:
                output_file = os.path.join(directory, '{}_kmer_{}.pickle'.format(group, self.kmer))
    ...
    def calculate(self, mutations_file):
        ...
        with read_function(mutations_file, mode) as read_file:
            ...
            for line in tqdm(fd):
                ...
                alt = line[parser.ALT]
                if self.groupby:
                    group_type = line[parser.GROUP_BY]
                else:
                    group_type = '0'
                        ...
                        if ref != '-' and alt != '-':
                            ...
                            if signature_ref[half_kmer] == ref:
                                ...
                                if n_ref == 0 and n_alt == 0:
                                    self.signatures[group_type]['counts'][(signature_ref, signature_alt)] += 1
                                    count += 1
                                else:
