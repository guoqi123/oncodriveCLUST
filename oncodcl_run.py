...
def write_results(results, output_file):
    ...
    :return: None
    """
    header = ['SYMBOL', 'SCORE_OBS', 'P-VALUE', 'CGC']
    with open(output_file, 'w') as fd:
        fd.write('{}\n'.format('\t'.join(header)))
        for gene_name, values in results.items():
            score_obs, pvalue, cgc = values
            fd.write('{}\t{}\t{}\t{}\n'.format(
                gene_name, score_obs, pvalue, cgc))
...
def run(regions_d, chromosomes_d, mutations_d,output_file, n_simulations, smooth_window, cluster_window, cores):
    ...
    with tqdm(total=len(genes)) as pbar:
        for gene, obs, pval in map(odf2.run_region, genes):
            pbar.update(1)
            CGC = gene in CGC_genes
            results_d[gene] = (obs, pval, CGC)
...
def main(input_file, output_file, regions_file, n_simulations, smooth_window, cluster_window, cores, log_level):
    ...
    trees, regions_d, chromosomes_d = odf.regions(regions_file)
