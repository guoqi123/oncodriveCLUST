...
def run_region(arguments):
    ...
    if os.path.isfile('signature.pickle'):
        ...
        empirical_pvalue = get_p(observed, sim_scores)

        analytical = ap.AnalyticalPvalue()
        analytical_pvalue, bandwidth = analytical.calculate(observed, sim_scores)

        return symbol, observed, empirical_pvalue, analytical_pvalue
